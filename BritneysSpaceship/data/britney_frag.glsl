...
vec3 light(in vec3 p, in vec3 dir, in vec3 n, in float hid) {
	...
	if (hid>.5) {
		...
		vec3 c=texture2D(iChannel0,mod(p.zy+vec2(.4,0.2),vec2(1.))).xyz*2.;
		col+=c*abs(.01-mod(p.y-iGlobalTime*.1,.02))/.01*ao;
		col*=max(0.,1.-pow(length(p2.yz-vec2(.25,1.)),2.)*3.5);
	} else{
		vec3 c=(texture2D(iChannel0,mod(p.zx*2.+vec2(0.5),vec2(1.))).xyz);
		c*=abs(.01-mod(p.x-iGlobalTime*.1*sign(p.x+1.),.02))/.01;
		col+=pow(getcol.x,10.)*.0000000003*c;
		col+=pow(max(0.,1.-getcol.y),4.)
        *pow(max(0.,1.-abs(1.-mod(p.z+iGlobalTime*2.,4.))),2.)
